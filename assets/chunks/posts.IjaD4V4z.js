import { D as DEFAULT_LOCALE, l as localizePath } from './paths.B3LgDFX2.js';
import { o as objectType, i as dateType, j as arrayType, k as stringType, A as AstroError, U as UnknownContentCollectionError, a as createComponent, R as RenderUndefinedEntryError, u as unescapeHTML, b as renderTemplate, l as escape, n as renderUniqueStylesheet, p as renderScriptElement, q as createHeadAndContent, r as renderComponent } from './astro/server.Dx2-getx.js';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content.D77ygiUa.js');
...
const __vite_import_meta_env__ = {"ASSETS_PREFIX": undefined, "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SITE": "https://alekschen.github.io", "SSR": true};
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets.BEFxlJQg.js').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules.gRc0ErBT.js');
...
function resolveTranslationKey(post) {
...
async function getPostList(filterDraft = true, locale = DEFAULT_LOCALE) {
  const posts = await getCollection("posts", ({ data }) => {
    const isNotDraft = filterDraft ? !data.draft : true;
    const matchesLocale = locale === "all" ? true : resolvePostLocale(data.locale) === locale;
...
async function getPostBySlug(slug, locale = DEFAULT_LOCALE) {
  const posts = await getCollection("posts", ({ data }) => {
...
async function getAllTags(locale = DEFAULT_LOCALE) {
  const posts = await getCollection("posts", ({ data }) => {
    return !data.draft && resolvePostLocale(data.locale) === locale;
  });
...
async function getPostsByTag(tag, locale = DEFAULT_LOCALE) {
  const allPosts = await getPostList(true, locale);
  return allPosts.filter(
    (p) => p.tags.some((t) => t.toLowerCase() === tag.toLowerCase())
  );
}
async function getPostAlternateLocalePaths(post) {
...
export { DEFAULT_OUTPUT_FORMAT as D, VALID_SUPPORTED_FORMATS as V, getPostBySlug as a, getPostAlternateLocalePaths as b, getCoverFromTags as c, buildPostSeoDescription as d, buildPostSeoKeywords as e, getCollection as f, getPostList as g, getPostsByTag as h, getAllTags as i, joinPaths as j, isRemotePath as k, DEFAULT_HASH_PROPS as l };
