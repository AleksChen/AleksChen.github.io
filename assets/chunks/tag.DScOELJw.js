...
import { g as getUiText, $ as $$Layout } from './Layout.CY2ISJX9.js';
/* empty css                         */
...
const $$Home = createComponent(($$result, $$props, $$slots) => {
  ...
  ) })}`)} </div> ${more && renderTemplate`<div class="w-full flex justify-center mt-8 mb-8"> <a${addAttribute(localizePath(`/${page + 1}`, locale), "href")} class="btn btn-primary px-8 py-3"> ${ui.home.loadMore} </a> </div>`} </div> </div> `, "operations": ($$result2) => renderTemplate`<div class="flex items-center h-full"> ${renderComponent($$result2, "TerminalToggle", $$TerminalToggle, { "locale": locale })} </div>` })} ${renderScript($$result, "/home/runner/work/AleksChen.github.io/AleksChen.github.io/src/components/Home.astro?astro&type=script&index=0&lang.ts")} `;
